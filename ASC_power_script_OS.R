...
# ------------------------------------------------------------------------------
# define function for model and extracting p value for psilo term
test_hypotheses <- function(data) {
  # fit model with only main effects (no interactions)
  model <- lmer(ASC_Score ~ Condition + Simple_Complex + Drug + (1 | Participant_Number), data = data)
  
  # extract summary
  model_summary <- summary(model)
  
  # tidy & filter psilo effects
  tidy_summary <- as.data.frame(model_summary$coefficients) %>%
    rownames_to_column(var = "Term") %>%
    dplyr::select(Term, Estimate, `Std. Error`, `Pr(>|t|)`) %>%
    filter(Term == "DrugPsilocybin")  
...
run_simulation <- function(n, repetitions) {
  
  simulation_results <- map_df(1:repetitions, ~{
    simulated_data <- simulate_data(n)       #simulate
    test_hypotheses(simulated_data) %>%      # fit model
      mutate(simulation = .x)                # add a simulation number
  })
...
# test with one data set
test_hypotheses(simulate_data(n = 30))

# run 1000 simulations
simulation_table <- run_simulation(n = 30, repetitions = 1000)


# calculate power
power_analysis <- simulation_table %>%
  group_by(Term) %>%
  summarise(Power = mean(`Pr(>|t|)` < 0.05, na.rm = TRUE))  # Calculate proportion of significant p-values
power_analysis